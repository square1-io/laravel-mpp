...
- **The discovery document now carries everything that `draft-payment-discovery-01` asks for.** It previously stated only prices. It can also state what an operation does, what to send to it, what it returns, and who operates the service. Nothing about `x-payment-info` changes. The package still derives it from the live router and from the request builders that mint the 402, so the document and the challenge cannot disagree on a price.
- Service-level fields are config, under `mpp.discovery`: the `x-service-info` extension of the draft, the rest of the OpenAPI `info` object, and `servers`. `title` follows `app.name`, and `servers` follows `app.url`.
- Documentation for one operation has eleven fields. Write them in any of three places, which the package merges field by field, with the place nearest the route winning: the `->discovery()` route macro, the new `#[DiscoveryInfo]` attribute, and `mpp.discovery.operations` for a route that you did not define. `hidden: true` keeps a payable route out of the document without making it free.
- **The package derives most of the document from the application.** The name of a route becomes its `operationId`. Path parameters and their `where()` constraints become typed parameters. A `FormRequest` becomes a request body, or a set of query parameters on a verb that carries no body. A docblock can become the summary and the description, through `mpp.discovery.docblocks`, which is off by default.
- `request` and `response` take a JSON Schema array, a full OpenAPI object, or a class name. A class states its schema through the new `Square1\Mpp\Discovery\ProvidesSchema`, through the `rules()` of a `FormRequest`, or through the types of its public properties. The last of those reads backed enums, nested objects, dates, and the collection types that a docblock states, such as `list<Scoreline>`. An application that returns data objects therefore writes no schema at all.
- **The document declares the response headers that the gate sets.** They are `Payment-Receipt`, `Payment-Session`, `WWW-Authenticate` and `Retry-After`, and they arrive with the 409 that the gate returns while a settlement is in progress. A site owner does not choose these headers and cannot remove them, so the package no longer asks for them. A header that you describe yourself wins.
- `mpp.discovery.include` lists free routes beside the paid ones, by route name, by `"GET /uri"` or by wildcard. Such a route carries no `x-payment-info` and no 402. The list is empty by default, because a broad pattern publishes your route table on an unauthenticated endpoint that registries crawl.
- `mpp.discovery.pipeline` takes `[Class::class, 'method']` stages. Each one receives the finished document and returns it, for any part of OpenAPI that the package does not model.
- The package serves the document with the two headers that the draft recommends, `Cache-Control: public, max-age=300` and `Access-Control-Allow-Origin: *`. The test suite validates the document against the JSON Schemas of the draft, which adds `opis/json-schema` as a dev dependency.

### Changed

- The exception messages, the log messages and the RFC 9457 `detail` text now follow the same plain-English standard as the documentation. No message carries a different meaning. One of them is visible to a client: the `detail` of the 409 that the gate returns while a settlement is in progress.

### Fixed

- A route with an optional parameter published an invalid path template, such as `/report/{year}/{month?}`. That path names a parameter `month?` and declares it nowhere. An optional Laravel parameter now becomes two OpenAPI paths, because an OpenAPI path parameter is always required.
