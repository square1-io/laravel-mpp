...
- `mpp.discovery.include` lists free, non-gated routes alongside the paid ones — a redirect, a status endpoint, the free tier of a paid one — matched by route name, `"GET /uri"` or `"/uri"` with `*` wildcards. They are documented like any other route but carry no `x-payment-info` and no `402`. Empty by default: a broad pattern publishes your route table on an unauthenticated endpoint that registries crawl.
- The test suite now validates the generated document against the draft's own JSON Schemas for `x-payment-info` and `x-service-info`, copied verbatim into `tests/Fixtures`. Both are `additionalProperties: false`, so this catches the well-meant extra key that a strict registry rejects — including one added through the pipeline, which is the one place an application can make the document non-conformant. Adds `opis/json-schema` as a dev dependency.
...
- An operation that declared its own responses no longer also gets a `200` it never returns. A route documented as a `307` redirect and a `404` was published with a spurious `200` alongside them. The `402` is still added to every payable operation whatever else it declared.
